...
With wind dir, moon phase, inHg, length guard, etc.
"""
import os
import sys
import random
...
CHANNEL = 1  # Normal broadcasts on channel 1
...
# Wind direction emojis (16-point compass)
WIND_ARROWS = [
    "↓", "↙", "←", "↖", "↑", "↗", "→", "↘",
    "↓", "↙", "←", "↖", "↑", "↗", "→", "↘"
]
...
def get_status(feels_num, hum_num, icon, wind, hum, wind_speed):
    # Prioritize wind if strong
    if wind_speed > 25:
        return f"WINDY 🌬️ {wind} — hold onto your hat!"

    if feels_num < 32:
        return f"BUNDLE UP 🥶 Feels like {feels_num}°F — frostbite risk!"

    if feels_num > 95:
        return f"HEAT ALERT 🔥 Feels like {feels_num}°F — stay hydrated!"

    if feels_num > 85:
        return f"HOT & HUMID 🥵 Feels like {feels_num}°F"

    if hum_num > 80:
        return f"HUMID & STICKY 💧 {hum} — muggy out there"

    # Exact match for rain-related icons (fixes substring false positive)
    rain_icons = {"🌧", "🌦", "⛈️", "☔", "🌧🌧"}
...
def send_to_mesh(message):
    log(f"Sending on channel {CHANNEL} ({len(message)} chars)")
    try:
